...
xSize compile_sequence(xParser * parser, const char_t * const regexp) {
    const char_t *sp = regexp;
    trace_enter();

    xSize start = MemSpace.len(parser->CHAR_SPACE);
    ...
    xSize n_plains = MemSpace.len(parser->CHAR_SPACE) - start;
